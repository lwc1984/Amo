...
### 5.3 显示布局（横屏 320×170）—— 小怪物的嘴

> **2026-08-20 重写。** 原设计是"左侧色块 + 右侧文字"的仪表盘式布局。后来这块屏
> 被嵌进 3D 打印的 Claude 像素风小怪物的嘴部开口（约 40×23mm），前提就变了：
> **它不是一块显示器，是一张脸。**

从开口尺寸能推出一条硬约束：40×23mm 上，2 米外能读到的只有**嘴型和颜色**，
文字必须走近看。这与 §7 的「6px 色条是核心，从 2 米读」是同一条原则，
只是载体从色条换成了嘴。

**状态词保留。** 嘴型确实也在表达状态，但**嘴型要学、文字不用学**：第一次看见
参差的红牙，没人知道那叫「没声儿了」。文字是那个锚点，学会之后才轮到嘴型在
余光里独立工作。

#### 两套排布

**张嘴态**（干着呢 / 该你了 / 没声儿了）—— 上下各 5 颗大方牙，文字在口腔里：
...
**闭嘴态**（摸鱼中 / 连不上）—— 一条唇线，文字在唇线上下：

```
              摸鱼中                              ← 28px，状态色
   ▄▄▄___▄▄▄___▄▄▄___▄▄▄___▄▄▄___▄▄▄___          ← 城墙状直角波
            amo-display                          ← 16px，白
```

嘴闭上了就不该还看见牙，所以闭嘴态是唇线而不是"两排快碰上的牙"。

#### 五种嘴型

| 状态 | 颜色 | 嘴 | 动作 |
|---|---|---|---|
| 干着呢 | `#3FBFD8` | 半开 | 咀嚼，380ms 一开合 |
| 该你了 | `#FFB020` | **张到最大** | 快速开合，260ms |
| 摸鱼中 | `#4A5B78` | 城墙状唇线 | 单向右滚，2.4s 一周期 |
| 没声儿了 | `#E2564A` | 牙齿参差 | **完全静止** |
| 连不上 | `#3A6EA5` | 压平的断线 | 慢闪，1.5s |

几条不是"手感"而是设计的取值：

- **waiting 与 running 的张合幅度刻意拉开**（12-18 对 24-36）。余光扫过时形状比
  颜色先被认出来；两者只差颜色，等于把这条信号压回单一维度。
- **stale 完全静止**。别的状态都在动，唯独它不动 —— 不动本身就成了信号，
  比任何闪烁都更像"出事了"。
- **nolink 慢闪而非静止**。它与 stale 都不正常，但不正常的方式不同：
  一个冻住了，一个还在重试。三种"不正常"必须长得不一样。
- **城墙用直角不用正弦**。像素风里本来就不该有曲线。方块高度必须 ≥ 2×振幅，
  否则高低两排接不上，看着是两排虚线而不是一道连续的墙。
- **唇线单向滚动，不来回摆**。来回摆看着像犹豫，单向滚才像"在慢慢晃悠"。
- **`连不上` 的颜色从 `#1E3A5F` 提亮到 `#3A6EA5`**。实测在 40×23mm 上暗到几乎
  看不见，而「断连绝不能渲染成正常」的前提是它得先被看见。

#### 亮度策略

LCD 背光是全屏均匀的，**画成黑色一分电都不省** —— 原设计"AMOLED 黑像素不通电"
的前提不成立。分档只能靠 LEDC 给 GPIO38 输出 PWM：等待态 100%，其余 55%。
